...
def test_standardize_batch():
    ...
    assert np.all(np.isclose(standardized_ft.mean(axis=1), 0))
    assert np.all(np.isclose(standardized_ft.std(axis=1), 1, atol=1e-4))
...
def test_align():
    X = asr.features.FeaturesExtractor.align(
        [np.ones([50, 10]), np.zeros([70, 10]), np.ones([60, 10])],
        default=9)
