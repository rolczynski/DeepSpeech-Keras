...
class Audio(Dataset):
    ...
    """
    def __init__(self, use_filesizes=True, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.sr = None
        self.use_filesizes = use_filesizes

    @classmethod
    def from_csv(cls, file_path: str, use_filesizes=True, **kwargs):
        ...
        return cls(references=references, use_filesizes=use_filesizes, **kwargs)
    ...
    def get_batch(self, index: int) -> Tuple[List[np.ndarray], List[str]]:
        ...
        batch_audio_data = [utils.read_audio(file_path) for file_path in paths]
