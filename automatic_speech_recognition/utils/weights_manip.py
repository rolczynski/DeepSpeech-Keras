...
def load_weights(model: keras.Model, model_chk: str, skip_on_fail: bool = False, verbose=True):
    """
    Loads weights from checkpoint. Weights are loaded by their names which allows to have different
    architectures in current model and checkpoint.
    There is a bug in tf which prevents code from working in tf 2.0. More info at
    ...
    skipped_layers = set()
    unused_saved_layers = {layer.name: layer.get_weights() for layer in saved_model.layers}
    ...
    if skipped_layers and not skip_on_fail:
        raise RuntimeError("Runtime skipped some layers while loading model. Enable verbose to know which layers."
...
def _print_layer_usage_info(skipped_layers, unused_saved_layers):
    ...
    logger.info(f"Didn't use {len(unused_saved_layers)} weights from saved model.")
